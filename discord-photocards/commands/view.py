...
class ViewBotCommand(BotCommand):
    ...
    def process(self):
        ...
        if len(message_data) == 1:
            owned_images = get_database_handler().get_owned_images(
                self.author_id, collection_name
            )
            ...
            if item_no in owned_images:
                return {
                    ...
                    "files": [
                        discord.File(
                            collection.image_paths[item_no],
                            filename=collection.image_paths[item_no].name,
                        )
                    ],
                }
            else:
                return {
                    ...
                    "files": [
                        discord.File(
                            collection.preview_path,
                            filename=collection.preview_path.name,
                        )
                    ],
                }
