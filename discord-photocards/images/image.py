...
def get_collection_picture(collection: Collection, unlocked_images: List[int] = []):
    ...
    canvas_width = (
        collection.images_per_row * collection.image_width
        + (collection.images_per_row + 1) * BORDER_SIZE
    )
    canvas_height = (
        collection.images_per_column * collection.image_height
        + (collection.images_per_column + 1) * BORDER_SIZE
    )
    ...
    for i in range(collection.num_items):
        ...
        preview_image = preview_image.resize(
            (collection.image_width, collection.image_height), Image.ANTIALIAS
        )
        image.paste(
            preview_image,
            (
